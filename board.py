...
class Board():
    def __init__(self, size, prob):
        self.size = size #size is the row column how big we want the board 
        self.prob = prob #bomb can set as probability
        self.lost = False
        self.won = False
        self.numClicked = 0
        self.numNonBombs = 0

        self.Board = [] #2D list for row column
    ...
    def handleClick(self,piece, flag):
        ...
        if (piece.getNumAround() != 0):
            return
        for neighbor in  piece.getNeigbors():
